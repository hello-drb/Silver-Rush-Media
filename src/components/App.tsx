  ...
  return (
    <div className="min-h-screen nyc-grid selection:bg-sr-rust selection:text-white">
        ...
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          >
            <img
              ...
              className={`transition-all duration-300 ${scrolled ? "h-10" : "h-14 brightness-0 invert"}`}
              referrerPolicy="no-referrer"
            />
            <span className={`font-display font-black tracking-tighter transition-all duration-300 ${scrolled ? "text-ink text-lg" : "text-white text-xl"}`}>SILVER RUSH MEDIA</span>
          </motion.a>
          ...
          <button
            className={`md:hidden transition-colors ${scrolled ? "text-ink" : "text-white"}`}
      ...
      <section className="relative min-h-screen flex items-center pt-28 md:pt-32 px-6 md:px-20 overflow-hidden bg-ink" id="hero">
          ...
          >
            ...
            <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="bg-sr-rust text-white px-12 py-6 rounded-none flex items-center gap-4 text-sm font-bold uppercase tracking-[0.2em] hover:bg-white hover:text-ink transition-all shadow-xl"
                id="hero-cta"
              >
                Let's Talk <ArrowRight size={20} />
              </motion.a>
              <a href="#setup" className="text-xs font-black uppercase tracking-widest flex items-center gap-2 text-white/70 hover:text-sr-rust transition-colors">
        ...
        {/* Background Accent */}
        <div className="absolute top-1/2 right-0 -translate-y-1/2 w-1/2 h-full bg-sr-teal/10 blur-[150px] rounded-full -z-10" />
      </section>
                ...
                <div className="max-w-4xl mx-auto">
                  <div className="flex items-start gap-8 md:gap-16 flex-col md:flex-row">
                    ...
                    <div>
                      ...
                      <button
                        onClick={() => toggleService(service.id)}
                        className="mt-6 text-xs font-bold uppercase tracking-[0.2em] text-sr-teal hover:text-white transition-colors flex items-center gap-1"
                      >
                        {isExpanded ? "Show Less" : "Show More"}
                        <ChevronDown size={14} className={`transition-transform ${isExpanded ? "rotate-180" : ""}`} />
                      </button>
            ...
            {projects.map((project: any, idx: number) => (
              ...
              >
                ...
                <div className="px-2">
                  ...
                  <p className="text-ink/80 leading-relaxed font-serif italic text-lg">{project.description}</p>
                </div>
              </motion.div>
            ))}
          </div>
      ...
      <section className="py-32 px-6 md:px-20 bg-white/30" id="contact">
        <div className="max-w-5xl mx-auto text-center">
          ...
          <p className="text-xl text-ink/80 mb-20 max-w-2xl mx-auto font-serif italic" id="contact-subtitle">
          ...
          {formSubmitted ? (
            ...
            >
              ...
              <p className="text-xl text-ink/80 font-serif italic">We're excited to learn more about your work. We'll be in touch within 48 hours.</p>
            </motion.div>
          ) : (
